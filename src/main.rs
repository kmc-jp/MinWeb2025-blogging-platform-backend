...
use MinWeb2025_blogging_platform_backend::{
    infrastructure::article_repository::InMemoryArticleRepository,
    presentation::handlers::article_handler::create_article_handler,
...
async fn main() {
    ...
    let app = Router::new().nest(
        "/api/articles",
        create_article_handler(ArticleUsecase::new(articles)),
    );
...
async fn get_all_articles(State(articles): State<Articles>) -> Json<Vec<Article>> {
