...
use crate::domain::models::article::Article;

pub async fn get_blog_text(uri: &str, user_name: &str, title: &str) -> Result<Option<Article>> {
    let collection = get_blog_collection(uri, user_name).await?;
    let filter = doc! { "title": title };
    collection.find_one(filter).await
}
async fn get_blog_collection(uri: &str, user_name: &str) -> Result<Collection<Article>> {
